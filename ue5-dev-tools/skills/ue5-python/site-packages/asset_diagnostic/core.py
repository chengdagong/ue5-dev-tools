...
class DiagnosticIssue:
    ...
    evidence: Optional[List[str]] = None  # List of evidence lines supporting the diagnosis
...
class DiagnosticResult:
    ...
    def print_report(self, verbose: bool = False):
        ...
        unreal.log("")
        unreal.log("=" * 80)
        unreal.log(f"DIAGNOSTIC REPORT: {self.asset_name}")
        unreal.log("=" * 80)
        unreal.log(f"Asset Path: {self.asset_path}")
        unreal.log(f"Asset Type: {self.asset_type.value}")
        unreal.log("")

        if self.summary:
            unreal.log(f"Summary: {self.summary}")
            unreal.log("")

        # Print metadata only in verbose mode
        if verbose and self.metadata:
            unreal.log("## ASSET INFO")
            for key, value in self.metadata.items():
                unreal.log(f"  {key}: {value}")
            unreal.log("")

        # Print issues by severity
        unreal.log("## ISSUES FOUND")
        unreal.log(
        ...
        if not self.issues:
            ...
            for severity in [
                ...
                if severity_issues:
                    unreal.log(f"### {severity.value}S ({len(severity_issues)})")
                    for idx, issue in enumerate(severity_issues, 1):
                        # Header line with actor name if available
                        if issue.actor:
                            unreal.log(f"  [{idx}] {issue.actor}")
                            unreal.log(f"      [{issue.category}] {issue.message}")
                        else:
                            unreal.log(f"  [{idx}] [{issue.category}] {issue.message}")
                        # Evidence lines
                        if issue.evidence:
                            unreal.log(f"      Evidence:")
                            for ev in issue.evidence:
                                unreal.log(f"        - {ev}")
                        # Suggestion
                        if issue.suggestion:
                            unreal.log(f"      Suggestion: {issue.suggestion}")
                    unreal.log("")

        unreal.log("=" * 80)
        if self.has_errors:
            unreal.log("[FAIL] Diagnostic completed with errors")
        elif self.has_warnings:
            unreal.log("[WARN] Diagnostic completed with warnings")
        else:
            unreal.log("[OK] Diagnostic completed successfully")
        unreal.log("=" * 80)
