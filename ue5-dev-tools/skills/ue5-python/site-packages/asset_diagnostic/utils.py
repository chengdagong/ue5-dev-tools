...
def aabb_intersects(origin1, extent1, origin2, extent2, epsilon=0.1):
    """
    Check if two axis-aligned bounding boxes intersect.

    Args:
        origin1: Center point of first AABB (unreal.Vector or tuple)
        extent1: Half-extents of first AABB (unreal.Vector or tuple)
        origin2: Center point of second AABB (unreal.Vector or tuple)
        extent2: Half-extents of second AABB (unreal.Vector or tuple)
        epsilon: Tolerance for floating point comparisons and near-touches (default 0.1 units)

    Returns:
        bool: True if AABBs intersect or nearly touch, False otherwise
    """
    # Convert to tuples if Vector objects
    if hasattr(origin1, "x"):
        o1 = (origin1.x, origin1.y, origin1.z)
    else:
        o1 = origin1

    if hasattr(extent1, "x"):
        e1 = (extent1.x, extent1.y, extent1.z)
    else:
        e1 = extent1

    if hasattr(origin2, "x"):
        o2 = (origin2.x, origin2.y, origin2.z)
    else:
        o2 = origin2

    if hasattr(extent2, "x"):
        e2 = (extent2.x, extent2.y, extent2.z)
    else:
        e2 = extent2

    # Check overlap on each axis (with epsilon tolerance for near-touches)
    for i in range(3):
        min1 = o1[i] - e1[i]
        max1 = o1[i] + e1[i]
        min2 = o2[i] - e2[i]
        ...
        # If gap between boxes is greater than epsilon, they don't intersect
...
def is_horizontal_orientation(actor, tolerance_degrees=5.0):
    """
    Check if an actor is oriented horizontally (facing forward, not tilted).

    Args:
        actor: UE5 Actor object
        tolerance_degrees: Maximum allowed deviation from horizontal (default 5.0)

    Returns:
        tuple: (is_horizontal: bool, pitch: float, roll: float)
            - is_horizontal: True if pitch and roll are within tolerance
            - pitch: Actor's pitch angle in degrees
            - roll: Actor's roll angle in degrees
    """
    try:
        rotation = actor.get_actor_rotation()
        pitch = rotation.pitch
        ...
        is_horizontal = abs(pitch) <= tolerance_degrees and abs(roll) <= tolerance_degrees
        return (is_horizontal, pitch, roll)
    except Exception:
        return (True, 0.0, 0.0)  # Assume horizontal if unable to get rotation


def get_actor_bounds(actor, only_colliding=True):
    """
    Get actor's bounding box origin and extent.

    Args:
        actor: UE5 Actor object
        only_colliding: If True, only considers colliding components (recommended for
                        physics/support detection). If False, includes all components
                        which may result in oversized bounds for Characters with
                        navigation/perception components.

    Returns:
        tuple: (origin: unreal.Vector, box_extent: unreal.Vector) or (None, None) if no bounds
    """
    try:
        # For Character/Pawn, use capsule bounds for more accurate collision representation
        if only_colliding and isinstance(actor, unreal.Character):
            capsule = actor.get_component_by_class(unreal.CapsuleComponent)
            if capsule:
                location = actor.get_actor_location()
                radius = capsule.get_scaled_capsule_radius()
                half_height = capsule.get_scaled_capsule_half_height()
                # Create extent from capsule dimensions
                extent = unreal.Vector(radius, radius, half_height)
                return (location, extent)

        origin, box_extent = actor.get_actor_bounds(only_colliding_components=only_colliding)
        if box_extent and (box_extent.x > 0 or box_extent.y > 0 or box_extent.z > 0):
